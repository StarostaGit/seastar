...
public:
    ...
    virtual future<size_t> write(uint64_t aligned_pos, const void* aligned_buffer, size_t aligned_len) = 0;
    virtual future<size_t> read(uint64_t aligned_pos, void* aligned_buffer, size_t aligned_len) = 0;
...
public:
    ...
    future<size_t> read(uint64_t aligned_pos, CharType* aligned_buffer, size_t aligned_len) {
        return _block_device_impl->read(aligned_pos, aligned_buffer, aligned_len);
    }

    template <typename CharType>
    future<size_t> write(uint64_t aligned_pos, const CharType* aligned_buffer, size_t aligned_len) {
        return _block_device_impl->write(aligned_pos, aligned_buffer, aligned_len);
    }
...
public:
   ...
   ~file_block_device_impl() override = default;
    ...
    future<size_t> write(uint64_t aligned_pos, const void* aligned_buffer, size_t aligned_len) override {
        return _file.dma_write(aligned_pos, aligned_buffer, aligned_len);
    }

    future<size_t> read(uint64_t aligned_pos, void* aligned_buffer, size_t aligned_len) override {
        return _file.dma_read(aligned_pos, aligned_buffer, aligned_len);
    }
