...
struct temporary_file {
    std::string path = "/tmp/block_device_test_file.XXXXXX";
    ...
    temporary_file& operator=(temporary_file&& tf) noexcept = delete;

    ~temporary_file() { unlink(path.data()); }
};
...
int main(int argc, char** argv) {
    ...
    return app.run(argc, argv, [&app] {
        return async([&] {
            ...
            const std::string& device_path = [&]() -> const std::string& {
                if (args.count("dev")) {
                    return args["dev"].as<std::string>();
                }

                tmp_device_file.emplace();
